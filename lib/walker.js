    ...
    function walker(app, resourceGroups, logging) {
        ...
        responses.map(function(value, index) {
            ...
            {
                case 'GET':
                    results.push(app.get(value.path, sendResponse(value.responses, logging)));
                    break;
                case 'POST':
                    results.push(app.post(value.path, sendResponse(value.responses, logging)));
                    break;
                case 'PUT':
                    results.push(app.put(value.path, sendResponse(value.responses, logging)));
                    break;
                case 'DELETE':
                    results.push(app['delete'](value.path, sendResponse(value.responses, logging)));
                    break;
                case 'PATCH':
                    results.push(app.patch(value.path, sendResponse(value.responses, logging)));
                    break;
                default:
                    results.push(void 0);
                    break;
    ...
    function sendResponse(responses, logging) {
            ...
            if ('prefer' in req.headers) {
                ...
                if (req.headers['prefer'] in responses) {
                    ...
                    if (logging) {
                        ...
                        winston.warn(`[${req.url}] Preferred response ${req.headers['prefer']} not found. Falling back to ${response.status}`);
                    }
                }
            }
